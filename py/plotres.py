...
def main():
    ...
    # ext = F04(Rv=3.1)
    # import astropy.units as u
    # gd71 = np.genfromtxt("/Users/jonatanselsing/Work/DATA/XSH/Crab_Pulsar/fGD71.dat")
    # redd = ext.extinguish(gd71[:, 0]*u.angstrom, Ebv=0.2580)
    # ext_corr = gd71[:, 1]#/redd
    # pl.plot(gd71[:, 0], ext_corr)

    wei = np.genfromtxt("../data/weighted_spectrum.dat")
    pl.plot(wei[:, 0], wei[:, 1])
    ...
    if True == True:
        ...
        for ii, OB in enumerate(OBs):
            ...
            # passbands = [
                ...
                # ]
            passbands = ["FORS1_U.dat", "FORS1_V.dat", "FORS1_B.dat", "FORS1_R.dat", "FORS1_I.dat", "FORS2_z.dat", "NACO_J.dat", "NACO_H.dat", "NACO_Ks.dat"]
            photometry = pd.read_csv("../data/Crab_phot.csv")

            vega_conv = pd.read_csv("~/github/iPTF16geu/data/passbands/VEGA_AB.csv")
            cmap1 = sns.cubehelix_palette(len(passbands), start=.2)
            cmap2 = sns.cubehelix_palette(len(passbands))
            for pp, ss in enumerate(passbands):
                ...
                dust_ext, ebv = correct_for_dust(vega["leff"].values, 83.633517, 22.01542)
                print(dust_ext, ebv)
                ...
                pl.errorbar(cwav, synmag_flux, xerr = [[cwave]], yerr = synmag_error,  fmt = 'o', color=cmap1[pp], zorder = 10, ms = 5, elinewidth=1.7, label = "%s = %s$^{+%s}_{-%s}$"%(pass_name, np.around(synmag, 1), np.around(e_u, 1), np.around(e_l, 1)))
                ...
                if meas_mag.shape[0] > 0:
                    ...
                    pl.errorbar(cwav, dust_ext*meas_flux, xerr = [[cwave]], yerr = [[(meas_flux_do, meas_flux_up)]],  fmt = 'o', color=cmap2[pp], ms = 15)
    # except:
        # pass
    # pl.loglog()
    pl.legend()
